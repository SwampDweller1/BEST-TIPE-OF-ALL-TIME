    ...
    while run:
        ...
        """else:
            for pieton in pietons:
                data_matrix[(j+1)*nbrpietons-2][0] = elapsed_time
                data_matrix[(j+1)*nbrpietons-1][0] = elapsed_time
                pietons.remove(pieton)
            for k in range(j*nbrpietons, (j+1)*nbrpietons):
        ...
        if elapsed_time>1:
            ...
            if int(elapsed_time)%3==0 and wait2>10:                             #Toutes les 10sec
            ...
            for pieton in pietons:
                ...
                if pieton.state==100:                                               # On recalcule à chaque itérations le nombre d'infectés
                    nb_infecte+=1
        ...
        Nombre_infecte.append([elapsed_time,nb_infecte])
