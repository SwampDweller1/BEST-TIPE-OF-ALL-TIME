...
prox_spat= int(input('Entrez la proximité spatiale (distance sociale): (usuel:50) '))
nr_experiments = 20
...
def main():
    ...
    def maj_pti(pieton):
        if abs(pieton.pos[0]-liste_pti[pieton.pti][0]) < 10 and abs(pieton.pos[1]-liste_pti[pieton.pti][1]) < 10 and pieton.pti != 4 and pieton.pti != 5 and pieton.pti != 6:
            r = random.randint(0,1000)
            if r<=20:                                                            #0.2% de chance de changer de pti à chaque itération
                pieton.pti_int=random.randint(0,4)                              #On change le point d'intérêt d'ORIGINE ! (pas pti_int)
            if r>=980:
                restore(pieton)
    ...
    def contact(L):
        ...
        if elapsed_time > 30:
            for i in L:
                if i.state < 100:
                    for j in L:
                        if (j != i and abs(j.pos[0] - i.pos[0]) < prox_spat and abs(j.pos[1] - i.pos[1]) < prox_spat and j.state == 100 and j.pos[0]<852):
                            nbrcontact += 1
                            M.append(i)
                        elif (j != i and abs(j.pos[0] - i.pos[0]) < 2*prox_spat and abs(j.pos[1] - i.pos[1]) < 2*prox_spat and j.state == 100 and j.pos[0]>852):
                            nbrcontact += 2
                            M.append(i)
        return [nbrcontact, M]
...
#------------------------------Simulation--------------------------------------#
    while run:
        ...
        if elapsed_time>1:                                                      #On attend de lancer le programme pour éviter les bugs d'initialisation
            ...
            for pieton in pietons:
                ...
                temps_dans_le_batiment=random.randint(10,20)                    #à changer à ta guise
    ...
    img = pyplot.imread("Shibuya.PNG")
    fig, ax = pyplot.subplots()
    ax.imshow(img)
    ax.scatter(topologiex,topologiey,s=10,label="Position de contamination",color='orange',edgecolors="black")
